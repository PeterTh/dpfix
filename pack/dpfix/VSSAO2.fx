/*
	...
	Modified further for inclusion in DPfix by Durante
*/

/***User-controlled variables***/
#define N_SAMPLES 32 //number of samples, currently do not change.

extern float aoRadiusMultiplier = 0.5; //Linearly multiplies the radius of the AO Sampling
extern float ThicknessModel = 13; //units in space the AO assumes objects' thicknesses are
...
static const float depthRange = 3000.0;
...
float readDepth(in float2 coord : TEXCOORD0) {
	...
	float d = (((col.b) + (col.g)*255 + (col.r)*256*255))/255.0;
	return d;
}
...
float4 ssao_Main(VSOUT IN) : COLOR0
{
	...
	ao = clamp(ao+0.2f, 0.0, 1.0);
	
	//return float4(depth/10.0f,depth/10.0f,depth/10.0f,1);

...
extern float aoRadiusMultiplier = 2.0; //Linearly multiplies the radius of the AO Sampling
extern float ThicknessModel = 10.0; //units in space the AO assumes objects' thicknesses are
extern float FOV = 65; //Field of View in Degrees
...
extern float aoStrengthMultiplier = 2.5;
#endif
...
static const float depthRange = 1000.0;
...
float readDepth(in float2 coord : TEXCOORD0) {
	...
	//float f = floor(col.g*255);
	//float d;
	//if(f < 127) d = (col.g) + (col.b)/255 + 1.0/255;
	//else if(f > 127) d = (col.g) + (col.b)/255;
	//else d = (col.g) + 1.0/255;
	//return 1-d;

	float d = (((col.b) + (col.g)*255 + (col.r)*256*255))/50.0;
	return d;
	//return 1.0 - clamp((((col.b) + (col.g)*256))/500.0, 0.0, 1.0);
	//return clamp(1.0 - (((col.b) + (col.g)*256 + (col.r)*256*255)/200.0), 0.0, 1.0);
	//float posZ = ((1.0-col.y) + 256.0*(1.0-col.x))/(5.0);
	//return ((2.0f * nearZ) / (nearZ + farZ - posZ * (farZ - nearZ)))*20.0;
	
	//float posZ = (col.z) + (col.y)*256.0 + (col.x)*256.0*256.0;
	//posZ = clamp(farZ-posZ, nearZ, farZ);
	//return (posZ/farZ)*(posZ/farZ)*(posZ/farZ)*(posZ/farZ);
}
...
float4 ssao_Main( VSOUT IN ) : COLOR0 {
	...
	//if(depth>0.5) return float4(1.0,0.0,0.0,1); 
	//else return float4(0.0,1.0,0.0,1); 
...
float4 Combine( VSOUT IN ) : COLOR0 {
	...
	//float3 c = tex2D(passSampler, IN.UVCoord/SCALE);
